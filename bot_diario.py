"""
=============================================================
  BOT DIARIO - CEDEARs en reMarkets (Primary API)
  Timeframe: Velas diarias | Historial: 6 meses
...
COMISION_IDA_VUELTA = 0.0076   # 0.33% + 0.05% x2
MAX_DIAS_ABIERTO   = 10

HORARIO_INICIO     = 11
HORARIO_FIN        = 17
INTERVALO_MINUTOS  = 5         # Analiza cada 5 minutos
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("bot_diario.log"),
...
posiciones         = {}   # {cedear: {precio_entrada, cantidad, stop_loss, take_profit, fecha_entrada, trailing_max}}
...
def obtener_historial(ticker: str) -> pd.DataFrame:
    """Velas diarias, 6 meses de historial desde Yahoo Finance."""
    try:
...
def evaluar_señal_compra(ind: dict) -> bool:
    ...
    # Filtro EMA 200: precio debe estar por encima
...
def evaluar_señal_venta(ind: dict) -> bool:
    """3 de 4 condiciones de salida por señal técnica."""
    if not ind:
        return False
...
def actualizar_trailing_stop(cedear: str, precio_actual: float):
    """Sube el stop loss si el precio subió, nunca lo baja."""
...
def registrar_en_excel(cedear, fecha_entrada, hora_entrada, precio_entrada,
    ...
    """Escribe la operación cerrada en el Excel de registro."""
    try:
        ...
        # Color verde si ganó, rojo si perdió
...
def cerrar_posicion(cedear: str, precio: float, motivo: str):
    """Cierra una posición y registra en Excel."""
    global capital_disponible, operaciones_total, ganancias_total, op_id_contador
    pos = posiciones[cedear]
...
def mostrar_resumen():
    log.info("=" * 65)
    log.info(f"[BOT DIARIO] PORTFOLIO - {datetime.now().strftime('%d/%m/%Y %H:%M')}")
...
def main():
    log.info("=" * 65)
    log.info("  BOT DIARIO - CEDEARs reMarkets")
    ...
    log.info(f"  Comision estimada: {COMISION_IDA_VUELTA*100:.2f}% ida y vuelta")
    log.info("=" * 65)
